...
namespace Dime.Scheduler.Connect
{
    ...
    {
        private const string Description = "Update the **category**, the visual indicator that controls the background color of the appointment, of an existing appointment on the planning board.";
        private const string Summary = "Appointment Category";

        [FunctionName(Functions.AppointmentCategory)]
        [OpenApiOperation(operationId: Functions.AppointmentCategory, tags: new[] { Tags.Appointment }, Description = Description, Summary = Summary)]
